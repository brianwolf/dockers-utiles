...
String _readRequiredEnv(String key) {
  ...
  if (value == null || value.isEmpty) {
    throw StateError(
      ...
      'Set it before running dartvader_auth.',
    );
  }
  return value;
}
...
/// Write the new credentials file to dartvader-credentials.json
...
Future<oauth2.Client> clientWithAuthorization() async {
  ...
  shelf_io.serveRequests(server, (request) {
    if (request.url.path == 'authorized') {
      ...
      server.close();
      return shelf.Response.ok(r'dartvader Authorized Successfully.');
    }
  ...
  Utils.stdoutPrint(
      'dartvader needs your authorization to upload packages on your behalf.\n'
...
Future<oauth2.Credentials?> readCredentialsFromLocal() async {
  ...
  if (!exists) {
    Utils.stdoutPrint('${Utils.credentialsFilePath} is not exist.\n'
        'Please run `dartvader_auth login` first');
    return null;
  }
